...
Generator:
[`noetl/core/runtime/nats_topology.py`](https://github.com/noetl/noetl/blob/main/noetl/core/runtime/nats_topology.py)
in the [`noetl/noetl`](https://github.com/noetl/noetl) repo.
Wiki: [`noetl/core/runtime/nats_supercluster`](https://github.com/noetl/noetl/wiki/nats_supercluster).

Sibling: a parameterized renderer/deployer playbook at
[`automation/infrastructure/nats_supercluster.yaml`](../../../automation/infrastructure/nats_supercluster.yaml)
deploys one cluster member at a time with overrideable parameters.
The committed YAML in this directory is the opinionated 2-cluster
canonical sample for local kind validation; the playbook is the
parameterized path for arbitrary deployments.

> **The supercluster is opt-in.** The single-node NATS deployment
> in `ci/manifests/nats/` remains the default `noetl k8s deploy`
> target; this directory's manifests are applied manually when an
> operator wants the multi-cluster topology.
